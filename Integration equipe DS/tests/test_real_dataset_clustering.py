...
def test_real_dataset_clustering():
    ...
    try:
        ...
        if response.status_code != 200:
            print("❌ Server not running. Start with: uvicorn main:app --reload --port 8080")
            return
        print("✅ Server is running!")
    except:
        print("❌ Cannot connect to server. Start with: uvicorn main:app --reload --port 8080")
        return
    ...
    try:
        ...
        with open(dataset_path, 'rb') as f:
            ...
            response = requests.post(
                "http://localhost:8080/groups/generate-clusters",
                files=files,
                timeout=180  # Increased timeout for large dataset
            )
    ...
    try:
        ...
        with open(dataset_path, 'rb') as f:
            ...
            response = requests.post(
                "http://localhost:8080/groups/generate-clusters-with-config",
                files=files,
                data=data,
                timeout=180
            )
